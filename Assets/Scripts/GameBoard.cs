...
public class GameBoard : MonoBehaviour
{
    ...
    public AudioClip consumedBonusAudioClip;
    ...
    void UpdateUI()
    {
        ...
        var highScorePlayerOne = int.Parse(highScoreText.text);

        if(playerOneScore > highScorePlayerOne)
        {
            PlayerPrefs.SetInt("playerHighScore", playerOneScore);
        } else if (playerTwoScore > highScorePlayerOne)
        {
            PlayerPrefs.SetInt("playerHighScore", playerTwoScore);
        }

        highScoreText.text = PlayerPrefs.GetInt("playerHighScore").ToString();
        int currentLevel;
    ...
    IEnumerator BlinkBoard(float delay)
    {
        ...
        // Reseta o jogo no próximo level
        shouldBlink = false;
        StartNextLevel();
    }
    ...
    public void StartConsumed(Ghost consumedGhost)
    {
        ...
        if (!didStartConsumed)
        {
            ...
            // Esconde os fantasmas consumidos
            ...
            // Toca o som do fantasma consumido
    ...
    public void StartConsumedBonusItem(GameObject bonusItem, int scorevalue)
    {
        ...
        consumedGhostScoreText.GetComponent<Text>().enabled = true;
        transform.GetComponent<AudioSource>().PlayOneShot(consumedBonusAudioClip);
    ...
    IEnumerator ProcessConsumedAfter(float delay, Ghost consumedGhost)
    {
        ...
        // Mostra os fantasmas consumidos
        ...
        // Pac-Man pode se mexer novamente
